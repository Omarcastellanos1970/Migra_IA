...
four runs but **three independent cases**: `other_brand` is `critical` with a
...
`other_brand_with_code` does change four answers —those about access to the
...
|---|---|---|---|
| `critical` | **85.0 → "Critical risk"** | open, 57 of 57 steps | Siemens S7-1500 (same brand) |
| `healthy` | **11.8 → "Low risk"** | not opened | — |
| `other_brand` | **85.0 → "Critical risk"** (same answers as `critical`) | open, with `cambio_marca` | OMRON Sysmac NX |
| `other_brand_with_code` | **69.2 → "High risk"** | open, with `cambio_marca` **and** the source program accessible: the porting route applies | OMRON Sysmac NX |
...
Both come from the `critical` scenario of `python _interactive_run.py`, so the
